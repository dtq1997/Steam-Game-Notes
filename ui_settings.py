...
class SettingsMixin:
    ...
    def _ui_api_key_settings(self):
        ...
        # ══════════ Steam Web API Key + 高级参数：左右两栏 ══════════
        ttk.Separator(win, orient=tk.HORIZONTAL).pack(fill=tk.X, padx=20, pady=8)

        bottom_row = tk.Frame(win)
        bottom_row.pack(fill=tk.X, padx=20, pady=(0, 5))

        # ── 左栏：Steam Web API Key ──
        left_col = tk.Frame(bottom_row)
        left_col.pack(side=tk.LEFT, fill=tk.BOTH, anchor=tk.N, padx=(0, 5))

        steam_frame = tk.LabelFrame(left_col, text="🎮 Steam Web API Key",
                                     font=("", 10), padx=10, pady=5)
        steam_frame.pack(fill=tk.X)

        steam_row = tk.Frame(steam_frame)
        steam_row.pack(fill=tk.X)
        steam_var = tk.StringVar(value=self._get_saved_key("steam_web_api_key"))
        steam_entry = tk.Entry(steam_row, textvariable=steam_var, width=26,
                                font=("", 9), show="•")
        ...
        show_s_btn.pack(side=tk.LEFT, padx=2)

        def clear_steam():
            steam_var.set("")
            self._clear_saved_key("steam_web_api_key")
        tk.Button(steam_row, text="🗑️", font=("", 9), relief=tk.FLAT,
                  command=clear_steam).pack(side=tk.LEFT, padx=1)
        ...
        tk.Label(steam_frame,
                 text="用于在线扫描游戏库\n从 steamcommunity.com/dev/apikey 获取",
                 font=("", 8), fg="#888", justify=tk.LEFT).pack(
                     anchor=tk.W, pady=(0, 3))

        # ── 右栏：高级参数 ──
        right_col = tk.Frame(bottom_row)
        right_col.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, anchor=tk.N,
                       padx=(5, 0))

        adv_frame = tk.LabelFrame(right_col, text="⚙️ AI 高级参数",
                                   font=("", 10), padx=8, pady=4)
        adv_frame.pack(fill=tk.X)

        # 从配置加载已保存的值（或使用默认值）
        _adv = self._config.get("ai_advanced_params", {})

        adv_grid = tk.Frame(adv_frame)
        adv_grid.pack(fill=tk.X)

        _adv_vars = {}  # 收集所有变量供保存用

        _adv_fields = [
            ("web_search_max_uses", "搜索次数上限",
             SteamAIGenerator.DEFAULT_WEB_SEARCH_MAX_USES,
             "每次生成最多搜几次 (1-10)，过多可能引入噪音"),
        ...
        def do_save_all():
            ...
            for key, (var, default) in _adv_vars.items():
                ...
                if val != default:
                    adv_dict[key] = val
            if adv_dict:
                self._config["ai_advanced_params"] = adv_dict
            elif "ai_advanced_params" in self._config:
                del self._config["ai_advanced_params"]
