...
def _worker_process_main(cmd_queue, result_queue, dylib_path, work_dir, app_id):
    ...
    # 5. 获取当前登录的 Steam 账号 ID
    logged_in_friend_code = None
    try:
        steam_user = None
        ...
        if steam_user:
            ...
            if steam_id64 and steam_id64 > 76561197960265728:
                logged_in_friend_code = str(steam_id64 - 76561197960265728)
    except Exception:
        pass
    ...
    # 7. 命令循环 — 处理 file_write / file_delete / shutdown
    while True:
        try:
            cmd = cmd_queue.get(timeout=1)
        except Exception:
            # queue.Empty 或其他异常，继续等待
            continue

        if cmd[0] == "shutdown":
            break

        elif cmd[0] == "file_write":
            _, filename, data = cmd
            try:
    ...
    try:
        ...
        pass
    # 进程退出 → OS 回收 dylib + IPC 连接 → Steam 检测到 Game Notes 停止
