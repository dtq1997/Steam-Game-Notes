...
from steam_data import (
    get_game_name_from_steam as _sd_get_game_name,
    get_game_details_from_steam as _sd_get_game_details,
    get_game_reviews_from_steam as _sd_get_game_reviews,
    format_game_context as _sd_format_game_context,
    format_review_context as _sd_format_review_context,
)
...
class SteamAIGenerator:
    ...
    def _call_anthropic(self, system_prompt: str, user_msg: str,
        ...
        联网搜索时强制使用「搜索-写作分离」两步法：
        Step 1（搜索阶段）：用轻量化提示词让模型专注于搜索和信息收集
        Step 2（写作阶段）：用完整提示词让模型专注于遵循格式/内容要求撰写笔记
        这彻底解决了搜索结果注入上下文后稀释格式指令的根本问题。
        """
        if not use_web_search:
            return self._call_anthropic_inner(
                system_prompt, user_msg, use_web_search=False)
...
⚠️ 最终整理必须使用简体中文，即使搜索结果是英文或日文。"""
    ...
    # ── Steam 数据获取（向后兼容委托，实际逻辑已移至 steam_data.py）──

    @staticmethod
    def get_game_name_from_steam(app_id: str) -> str:
        """向后兼容：委托给 steam_data.get_game_name_from_steam()"""
        return _sd_get_game_name(app_id)

    @staticmethod
    def get_game_details_from_steam(app_id: str) -> dict:
        """向后兼容：委托给 steam_data.get_game_details_from_steam()"""
        return _sd_get_game_details(app_id)

    @staticmethod
    def format_game_context(details: dict) -> str:
        """向后兼容：委托给 steam_data.format_game_context()"""
        return _sd_format_game_context(details)

    @staticmethod
    def get_game_reviews_from_steam(app_id: str, num_per_lang: int = 10) -> dict:
        """向后兼容：委托给 steam_data.get_game_reviews_from_steam()"""
        return _sd_get_game_reviews(app_id, num_per_lang)

    @staticmethod
    def format_review_context(reviews_data: dict,
                              max_reviews: int = 8,
                              max_chars_per_review: int = 300) -> str:
        """向后兼容：委托给 steam_data.format_review_context()"""
        return _sd_format_review_context(reviews_data, max_reviews, max_chars_per_review)
